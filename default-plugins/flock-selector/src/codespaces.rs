...
impl Codespace {
    pub fn state_kind(&self) -> StateKind {
        match self.state.as_str() {
            ...
            "Starting" | "Provisioning" | "Queued" | "Awaiting" | "Rebuilding"
            | "ShuttingDown" | "Exporting" | "Updating" => StateKind::Busy,
            _ => StateKind::Unknown,
        }
    }
}
...
This API operation needs the "codespace" scope. To request it, run:  gh auth refresh -h github.com -s codespace"#;
    ...
    tab name="terminal" {
        ...
        let tab_names: Vec<Option<String>> =
            layout.tabs().iter().map(|(name, _, _)| name.clone()).collect();
        assert_eq!(
            tab_names,
            vec![
                Some("ai".to_owned()),
