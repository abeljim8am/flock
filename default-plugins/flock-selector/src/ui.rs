    ...
    match mode {
        ...
        PickerMode::Projects => {
            ...
            for (k, idx) in (scroll..visible_end).enumerate() {
                ...
                let show_path = name_counts
                    .get(r.project.name.as_str())
                    .copied()
                    .unwrap_or(0)
                    > 1;
...
fn coder_error_spans(err: &CoderError, p: &Theme) -> Vec<Span> {
    let msg = match err {
        ...
        CoderError::NotConfigured => {
            " Coder deployment not configured — run: coder login <url>".to_owned()
        },
        CoderError::NotAuthenticated => {
            " Coder authentication required — run: coder login".to_owned()
        },
