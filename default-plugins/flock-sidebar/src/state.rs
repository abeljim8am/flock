...
impl PaneAgentState {
    ...
    pub fn effective_agent_label(&self) -> Option<String> {
        self.hook_authority
            ...
            .or_else(|| {
                self.detected_agent
                    .map(|a| detect::agent_label(a).to_string())
            })
    }
    ...
    fn apply_screen_signals(
        ...
        self.fallback_visible_idle = detection.visible_idle && fallback_state == AgentState::Idle;
...
mod tests {
    ...
    fn hook_authority_can_override_with_unknown_agent_label() {
        ...
        assert_eq!(pane.detected_agent, Some(Agent::Pi));
        assert_eq!(
            pane.effective_agent_label().as_deref(),
            Some("custom-agent")
        );
        assert_eq!(pane.state, AgentState::Working);
    }
    ...
    fn background_completion_marks_unseen() {
        ...
        assert!(
            !pane.seen,
            "an unfocused completion should read as Done-unseen"
        );
    }
    ...
    fn completion_while_focused_stays_seen() {
        ...
        assert!(
            pane.seen,
            "a completion the user is watching is already seen"
        );
    }
