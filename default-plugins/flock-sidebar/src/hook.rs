...
// Remote daemon versions used to arrive here over a `flock-daemon-version`
// pipe. They now ride `SessionInfo.remote_panes[..].health`, written by the
// bridge as a sidecar file and re-read by the server every tick — so the
// picture self-clears when the condition ends, survives a plugin reload, and
// is visible from every session rather than only the one that got the message.
