...
fn is_claude_selector_chrome_line(line: &str) -> bool {
    ...
    let numbered_option = trimmed.split_once('.').is_some_and(|(number, _)| {
...
pub fn identify_agent_from_command(command: &[String]) -> Option<Agent> {
    ...
    if is_generic_runtime_or_shell(&normalized_effective) {
        if let Some(name) =
            wrapped_agent_name_from_runtime_argv(&normalized_effective, Some(command))
        {
...
mod tests {
    ...
    fn command_does_not_identify_env_wrappers_as_agents() {
        assert_eq!(
            identify_agent_from_command(&cmd(&["devenv", "shell"])),
            None
        );
        assert_eq!(identify_agent_from_command(&cmd(&["nix", "develop"])), None);
    }
    ...
    fn node_wrapped_claude_js_is_identified() {
        let argv: Vec<String> = [
            "node",
            "/usr/lib/node_modules/@anthropic-ai/claude-code/claude.js",
        ]
        .iter()
