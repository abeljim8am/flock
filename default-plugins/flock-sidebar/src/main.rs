                        ...
                        match coder::parse_snapshot(&String::from_utf8_lossy(&stdout)) {
                            Ok(snapshot) => {
                                if let Some(identifier) =
                                    context.get(coder::SNAPSHOT_CONTEXT_KEY)
                                {
    ...
    fn pipe(&mut self, pipe_message: PipeMessage) -> bool {
        ...
        if pipe_message.name == coder::SNAPSHOT_PIPE_NAME {
            ...
            let mut snapshot = coder::Snapshot::from_states(
                now_millis(),
                self.current_session_name().unwrap_or_else(|| "flock".to_owned()),
                &self.last_published,
            );
...
impl State {
    fn pane_is_focused(&self, pane_id: PaneId) -> bool {
        let active_tab = self.tabs.iter().find(|tab| tab.active).map(|tab| tab.position);
        active_tab
    ...
    fn maybe_poll_coder_snapshot(&mut self, now: Instant) {
        ...
        if self.last_coder_snapshot_poll.is_some_and(|last| {
            now.duration_since(last).as_secs_f64() < CODER_SNAPSHOT_POLL_SECS
        }) {
            return;
        }
